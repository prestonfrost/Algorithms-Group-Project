...
int merge(int array[], int const left,
          int const mid, int const right)
{
    ...
    int comparisons = 0; // Initialize the counter for comparisons
              ...
              int const end)
{
    ...
    comparisons += mergeSort(array, begin, mid);
    comparisons += mergeSort(array, mid + 1, end);
    comparisons += merge(array, begin, mid, end); // Update comparisons with the number of comparisons in merge
    return comparisons;
}
...
int main() {
    ...
    for (int size : array_sizes) {
        ...
        for (auto& [dataset_name, generate_func] : datasets) {
            ...
            auto [execution_time, comparisons] = measure_time_and_comparisons([&](std::vector<int>& array) { return mergeSort(array.data(), 0, size - 1); }, r_array);
