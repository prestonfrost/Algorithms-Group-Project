...
void quicksort(std::vector<int>& arr, int low, int high, int& comparisons) {
    if (low < high) {
        ...
        quicksort(arr, low, partition_index - 1, comparisons);
        quicksort(arr, partition_index + 1, high, comparisons);
    }
}

int quick_sort(std::vector<int>& arr) {
    int comparisons = 0;
...
int main() {
    ...
    for (int size : array_sizes) {
        ...
        for (auto& [dataset_name, generate_func] : datasets) {
            ...
            auto [execution_time, comparisons] = measure_time_and_comparisons(quick_sort, r_array);
